...
function (_React$Component) {
  ...
  _createClass(ScrollTransition3D, [{
    ...
    value: function getSlideStyles(index, positionValue) {
      var currentItem = this.refs[index];
      var prevTargetPosition = currentItem ? parseInt(currentItem.style.left.replace('px', ''), 10) : 0;
      var targetPosition = this.getSlideTargetPosition(index, positionValue);
      var transformScale = this.getTransformScale(index);
      var disableTransition = Math.abs(targetPosition - prevTargetPosition) > this.props.slideWidth + this.props.cellSpacing;
      return {
        ...
        transition: "left ".concat(disableTransition ? '0s' : '0.4s', " ease-out, transform 0.4s ease-out, opacity ").concat(disableTransition ? '0s' : '0.8s', " ease-out"),
        verticalAlign: 'top',
